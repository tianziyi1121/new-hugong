...
var objfunction;
var timeflag = false;
var timerTitle = '该时间已预约，请选择其他时间';
var timerTitleFlag = 1 
let timerNumber = 0
Component({
  properties: {},
  /** 组件的初始数据 */
  data: {
    ...
    timeList: [
      // {
      //   service_begin_time: 1571673600,
      //   service_end_time: 1571760000
      // },
      // {
      //   service_begin_time: 1571932800,
      //   service_end_time: 1572105600
      // }
    ]
  },
  attached() {
  ...
  methods: {
    ...
    kaiSHiyun: function () {
      ...
      panXZhong.xainData
      panXZhong.xainData.map(item => {
        this.data.timeList.map(itemData => {
          (item.getTime/1000 +10800) > itemData.service_begin_time && item.getTime/1000 < itemData.service_end_time ? item.timerFlag = true : ''
        })
      })
      xianxuNZ.push(panXZhong)
    },

    getDataTime(timer, data, flag, num) {
      this.data.timeList.map(res => {
        if (!flag) {
          if (timer / 1000 < res.service_begin_time && data / 1000 >= res.service_end_time) {
            xuanZheData.splice(1, 1)
            this.setData({
              xuanZheData: xuanZheData,
            })
            timerTitleFlag = 1
            timerTitle = '该时间已预约，请选择其他时间'
            timeflag = true
            timerNumber = 1
          } else {
            timeflag = false
          ...
          if (timer / 1000 < res.service_begin_time && data / 1000 > res.service_begin_time) {
            xuanZheData.splice(1, 1)
            this.setData({
              xuanZheData: xuanZheData,
            })
            ...
            timeflag = true
            timerNumber = 1
          } else if (num == 2 && timer / 1000 <= res.service_end_time && data / 1000 >= res.service_end_time) {
            timerTitleFlag = 1
            timerTitle = '该时间已预约，请选择其他时间'
            timeflag = true
            timerNumber = 1
    ...
    dianXuan: function (e) {
      ...
      if (xuanZheData.length > 0 && xuanZheData[0].getTime != data.time){
        wx.setStorageSync('timerFlagLf', false)
        xuanZheData[0].getTime < data.time ? this.getDataTime(xuanZheData[0].getTime,  data.time, data.flag, 1) : this.getDataTime(data.time, xuanZheData[0].getTime, data.flag, 2)
      } else if (xuanZheData.length > 0 && xuanZheData[0].getTime == data.time){
        wx.setStorageSync('timerFlagLf', true)
        timeflag = true
        xuanZheData = []
        this.setData({
          xuanZheData: xuanZheData
        ...
        timerNumber = 0
        timerTitleFlag = 1
        data.flag ? timeflag = true : timeflag = false
      }

      if (timeflag || timerNumber == 1){
        timerTitleFlag == 2 ? '' : toolTip.noPhotoTip(timerTitle)
        timerNumber = 0
        return false
      }
      var shiJIan = data.shi + '-' + data.day
        ...
        if (xuanZheData.length < 2) {
          if (xuanZheData.length >= 1) {
            ...
            if (new Date(twoZHI).getTime() < new Date(firstZHI).getTime()) {
              ...
              xuanZheData.splice(0, 0, { xuanShiJian: data.shi, xuanDayShi: data.day, getTime: shiJIanHaoMiao })
              ...
              xuanZheData.push({ xuanShiJian: data.shi, xuanDayShi: data.day, getTime: shiJIanHaoMiao, chaDay: parseInt(cha / 86400000) })
              delete xuanZheData[0].chaDay;
            }
          }else {
            xuanZheData.push({ xuanShiJian: data.shi, xuanDayShi: data.day, getTime: shiJIanHaoMiao })
          }
        }else {
          var firstZHI = xuanZheData[0].xuanShiJian + '-' + xuanZheData[0].xuanDayShi
          firstZHI = firstZHI.replace(new RegExp("-", "gm"), "/")
          ...
          if (new Date(twoZHI).getTime() < new Date(firstZHI).getTime()) {
            ...
            var cha = new Date(twoZHI).getTime() - new Date(firstZHI).getTime()
            xuanZheData.splice(1, 1, { xuanShiJian: data.shi, xuanDayShi: data.day, getTime: shiJIanHaoMiao, chaDay: parseInt(cha / 86400000) })
            delete xuanZheData[0].chaDay;
          }
        }
      }
      ...
      if (xuanZheData.length == 2) {
        xuanZheData[0].text = '开始'; xuanZheData[1].text = '结束'
      }else{
        if (xuanZheData[0]) {
